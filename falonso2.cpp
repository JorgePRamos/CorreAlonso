...
void semtoRed(int sem) {
    ...
    PeekMessageA( & test_msg, NULL,401+ 303 - 2 * sem, 303 - 2 * sem, PM_REMOVE);//ipc_Nowait
    PeekMessageA( & test_msg, NULL,401+ 303 - 2 * sem - 1, 303 - 2 * sem - 1, PM_REMOVE);//ipc_Nowait
    luzSem(sem, AMARILLO);
...
void avance_controlado(int * carril, int * desp, int color, int v) {
    ...
    PeekMessageA( & test_msg, NULL,401+ pos_cambio + 1, pos_cambio + 1, PM_REMOVE);//IPC_NOWAIT
    ...
    if (!(posOcup( * carril, ( * desp + 1) % 137))) {
        ...
        if ( * desp == 21 && * carril) {
            if ((estadoSem(VERTICAL) == ROJO || estadoSem(VERTICAL) == AMARILLO)) {
                ...
                leaveCritic("critica", 1);
                if (GetMessage( & uMsg, NULL,401+ 300, 300) == -1) {
            ...
            if ((estadoSem(VERTICAL) == ROJO || estadoSem(VERTICAL) == AMARILLO)) {
                ...
                leaveCritic("critica", 1);
                if (GetMessage( & uMsg, NULL,401+ 301, 301) == -1) {
                    PERROR("[GetMessage] pausa Sem");
                    raise(SIGINT);
                }
            ...
            if (estadoSem(HORIZONTAL) == ROJO || estadoSem(HORIZONTAL) == AMARILLO) {
                ...
                leaveCritic("critica", 1);
                if (GetMessage( & uMsg, NULL,401+ 302, 302) == -1) {
                    PERROR("[GetMessage] pausa Sem");
                    raise(SIGINT);
                }
            ...
            if (estadoSem(HORIZONTAL) == ROJO || estadoSem(HORIZONTAL) == AMARILLO) {
                ...
                leaveCritic("critica", 1);
                leaveCritic("critica_salida", 1);
                if (GetMessage( & uMsg, NULL,401+ 303, 303) == -1) {//302¿?
                    PERROR("[GetMessage] pausa Sem");
                    raise(SIGINT);
                }
                enterCritic("sem_cruze", 1);
        ...
        if (!posOcup(! * carril, cambio_carril_cal( * desp, * carril))) {
            ...
            leaveCritic("critica", 1);
            //fprintf(stderr, "Color (%d) [%d] Suelto la seccion critica \n",color, getpid());
            if (GetMessage( & uMsg, NULL,401+ ( * desp + * carril * 137) + 1, ( * desp + * carril * 137) + 1) == -1) {
                PERROR("[GetMessage] pausa Sem");
                raise(SIGINT);
            }
            ...
            if (n != 1) {
                if (miIndice != n) {
                    ...
                    if (miIndice != 1) {
                        ...
                        if (GetMessage( & uMsg, NULL,401+ miIndice, 100+miIndice) == -1) {
                            PERROR("[GetMessage] pausa Sem");
                            raise(SIGINT);
                        }

                    }
                    if (PostThreadMessageA(GetCurrentThreadId(),401 + 100+miIndice + 1, 0, 0) == FALSE)
                        PERROR("Error PostMsg");
                    raise(SIGINT);

                    if (GetMessage( & uMsg, NULL,401+ 500 + miIndice, 500 + miIndice) == -1) {
                        PERROR("[GetMessage] pausa Sem");
                        raise(SIGINT);
                    }
                    ...
                    if (miIndice == n) {
                        if (GetMessage( & uMsg, NULL,401+ 600, 600) == -1) {
                            PERROR("[GetMessage] pausa Sem");
                            raise(SIGINT);
                        }
                    }
